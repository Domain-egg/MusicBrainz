...
const AlbumComponent = (props) => {
    ...
    fetch('http://coverartarchive.org/release-group/' + props.mbid).then(response => response.json())
        .then((img) => {
            setUri(img.images[0].image.toString())
        }).catch((error) => {
            console.error(error + ' while loading image of: ' + props.title)
    });

    return(
        //set the onPress to a function that was defined in the Parent View
        <TouchableHighlight style={styles.album} onPress={props.onPress}>
            <ImageBackground style={styles.imageContainer}  imageStyle={{ borderRadius: 10}} resizeMode='cover' source={{uri: uri}}>
                <Text style={styles.albumName}>{props.title}</Text>
            </ImageBackground>
        </TouchableHighlight>
    )
