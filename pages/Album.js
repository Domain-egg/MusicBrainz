...
export default function Album({GlobalState, route}) {
    ...
    React.useEffect(()=>{
        ...
        setFavorite(GlobalState.favorites.includes(dataSource))
    }, );
                    ...
                    <TouchableHighlight>
                        <Icon
                            ...
                            onPress={() => {
                                ...
                                var buffer = GlobalState.favorites;
                                var imgBuffer = GlobalState.favoritesImg;
                                if(buffer.includes(dataSource)) {
                                    setFavorite(false);
                                    if(buffer.indexOf(dataSource)==0) {
                                        buffer.shift()
                                        imgBuffer.shift()
                                    }else{
                                        buffer.splice(buffer.indexOf(dataSource),buffer.indexOf(dataSource))
                                        imgBuffer.splice(buffer.indexOf(dataSource),buffer.indexOf(dataSource))
                                    }
                                }else {
                                    setFavorite(true);
